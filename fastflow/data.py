...
class FeaturesTrainDataset(Dataset):
    def __init__(self, path_to_embs, layers, shapes):
        ...
        for layer, shape in zip(layers, shapes):
            ...
            self.features[layer] = np.memmap(filename=layer_file_path, mode='r', dtype='float32', shape=shape)
...
class FeaturesDatasetOOD(Dataset):
    def __init__(self, path_to_in_distr_embs, path_to_out_distr_embs, layers, shapes):
        ...
        for layer, shape in zip(layers, shapes):
            ...
            layer_file_path_out = path_to_out_distr_embs.replace('_'.join(layers), layer)
            self.features_in[layer] = np.memmap(filename=layer_file_path_in, mode='r', dtype='float32', shape=shape)
            self.features_out[layer] = np.memmap(filename=layer_file_path_out, mode='r', dtype='float32', shape=shape)
